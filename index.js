...
//1. İlk fonksiyonda skor fonksiyonun içerisinde güncellenmekte fakat skor2'de skor globalde tanımlanmış, skor fonksiyon scopeunun içinde güncelleniyor.
//2. İlk fonksiyon closure kullanıyor. Kendi içersindeki bir şeye dışarıdan erişebilmemizi sağlıyor.
//3. Eğer bu programı farklı farklı skorlar tutabilmek için kullanıcak isem skor1 closure'a sahip olduğu için onu tercih ederim.
//Ancak tek bir skor tutulucak ise ikinci fonksiyon daha kolayımıza gelir.
...
function macSonucu(callback, ceyrekSayisi) {
  let EvSahibi = 0;
  let KonukTakim = 0;
  for (let i = 1; i <= ceyrekSayisi.length; i++) {
    EvSahibi += callback();
    KonukTakim += callback();
  }
  return {
